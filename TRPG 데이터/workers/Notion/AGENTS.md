...
>
> **Do not use sync state as an admin command input.** The `state` argument is runtime-owned pagination/change cursor state. Likewise, CLI `--context` is only for resuming a previous preview's `nextContext`; it is not arbitrary input for a real sync. UUID-driven admin actions belong in tool/API/CLI management paths, while syncs should derive changes from the upstream source.
...
- `npm run worker:deploy`: build and publish capabilities. In the remote development environment `/shared` is a `v9fs` mount; direct `ntn workers deploy --no-git` from that mount corrupts uploaded source bytes. The project deploy script stages only deployment files under the OS-native temp filesystem and runs `ntn workers deploy --no-git` there. Use this command instead of direct deploy from `/shared`.
...
### TRPG settings admin commands

- `npm run db:list`: list current synced setting rows.
- `npm run db:find -- --uuid <uuid>`: exact UUID lookup.
- `npm run db:get -- --uuid <uuid>`: exact UUID lookup followed by page retrieval.
- `npm run db:delete -- --uuid <uuid>`: destructive target dry-run only; prints the exact row and makes no change.
- `npm run db:delete -- --uuid <uuid> --yes`: trash the exact single matched page. UUID is mandatory; zero or multiple matches abort.
- `npm run db:resync -- --uuid <uuid>`: currently limited to the configured single test source and triggers `settingsBackfill`.

`settingsDelete` is also exposed as a write tool for Custom Agents. It resolves exactly one row by UUID before trashing it. Tool execution through `ntn workers exec` requires a separately configured `NOTION_API_TOKEN`; Custom Agent invocation receives an authenticated `context.notion` automatically.
