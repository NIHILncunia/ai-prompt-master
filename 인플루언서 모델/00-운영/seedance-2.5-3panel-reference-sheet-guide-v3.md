# Seedance 2.5 3패널 캐릭터 레퍼런스 시트 지침 v3
...
`FACE MASTER + Morphology Card → 공식 신체 수치 + BODY-FRONT/SIDE/BACK + Body Signature → 승인 기본 복장 → 3패널 시트 규칙`

- 얼굴은 FACE MASTER와 Morphology Card를 따른다.
- 키, 체형, 정면·측면·후면 실루엣은 공식 신체 수치와 `BODY-FRONT / BODY-SIDE / BODY-BACK` 3분리 BODY MASTER를 따른다.
- 3분리 BODY MASTER의 승인 실루엣과 기본 복장은 새로 설계하거나 재해석하지 않는다. Seedance BASE 시트는 이미 승인된 신체 실루엣과 기본 복장을 동일 규칙의 3패널 형식으로 재배치하는 작업이다.
- 의상 때문에 체형 자체가 바뀌지 않게 Body Signature를 적용한다.
- FULL MASTER는 전체 인상 보조용으로만 사용한다.
- BASE 시트에서는 승인 기본 복장과 기본 헤어를 고정한다. BASE 승인 뒤에만 복장과 헤어를 작업별 가변 슬롯으로 교체할 수 있다.
- 레이아웃 참고가 필요할 때 MODEL-001 예린의 승인 Seedance 시트를 **FORMAT REFERENCE**로 사용할 수 있다. 이 경우 예린의 얼굴·체형·의상을 복제하지 않고 패널 구조와 배치만 참조한다.

## 2.1 초도 BASE 시트와 기본 복장 고정

이 절은 최초 Seedance 시트의 복장 규칙이다. 다른 가변 의상 지시보다 우선한다.

- 각 모델의 **첫 번째** Seedance 2.5 3패널 시트는 반드시 `BASE` 버전이다.
- BASE 시트는 승인된 `BODY-FRONT + BODY-SIDE + BODY-BACK` 앵커의 기본 복장을 정확히 유지한다. 상의·하의·신발 또는 맨발 상태·색·트림·소재·핏·승인된 의류 마크의 존재 여부를 CASUAL 등 다른 의상으로 교체하지 않는다.
- 루네바 등록 피팅 BASE에서는 제품 정본의 상의 목둘레·암홀·하단 검은 바인딩, 하의 앞·뒤 허리 밴드와 양쪽 밑단의 검은 마감을 모두 유지한다. 로고 위치만 맞고 하단 띠·바인딩·밑단 마감이 빠진 패널은 BASE로 승인하지 않는다.
- BASE 시트의 목적은 영상 생성에 전달할 최초 의상 포함 정체성 기준을 고정하는 것이다. 한 모델의 일반적인 패션 룩을 새로 제안하는 작업이 아니다.
- 복장 자체에 승인된 공식 마크가 있을 때만, 그 마크를 의상 구성요소로 정확히 유지할 수 있다. 시트의 캔버스 장식·워터마크·텍스트 로고는 항상 금지다. 정확한 마크 자료가 없으면 문자처럼 보이는 임의 로고를 만들지 않는다.
- `CASUAL`, `STAGE`, `FASHION`, `PRODUCT` 시트는 BASE 승인 후에 별도 파생 시트로 만든다. 기존 CASUAL/STAGE 시트가 있어도 BASE 시트를 대신하지 않는다.
- BASE 파일명은 `MODEL-[ID]-MOTION-REFSHEET-BASE-v01.png`이다.
...
- 최초 생성은 반드시 BASE 복장 시트다.
- BASE 시트가 승인되기 전에는 CASUAL·STAGE·FASHION·PRODUCT 시트로 초도 정체성 시트를 대체하지 않는다.
- BASE 시트의 의상은 BODY-FRONT / BODY-SIDE / BODY-BACK의 승인 기본 복장과 동일해야 한다.
...
- 두 전신 패널의 바깥쪽 어깨 상단선은 정확히 같은 수평선에 있어야 한다.
...
- 얼굴을 최대한 크게 담는 정면 두상 클로즈업.
- 정수리부터 턱까지를 우선해 크게 담는다. 턱 아래와 나머지 신체는 잘려도 된다.
- 눈높이 카메라.
...
- 카메라 정면 응시.
- 과도한 고개 기울기 금지.
- PANEL 1·2와 동일한 헤어 세팅을 유지한다. PANEL 3은 두상 전용이므로 어깨·목 주변의 의상이나 나머지 신체는 보여야 할 필수 정보가 아니다.
...
- exact same horizontal outer shoulder-top line
- same shoulder width
- same torso length
- same waist height
- same hip width
...
`The outer shoulder-top line of the front and rear full-body panels must be on exactly the same horizontal line.`
...
BASE 승인 이후의 파생 시트에서만, 복장은 촬영·영상 작업별로 새로 정의한다.
...
현재 성공 사례인 MODEL-001 차예린의 승인 시트를 레이아웃 참고로 사용할 수 있다. 이들은 BASE 규칙 도입 전의 CASUAL/STAGE 사례이므로, **3패널 구조만** 참고하고 초도 복장 기준으로 사용하지 않는다.
...
OUTFIT:
[FOR A FIRST SHEET: THE APPROVED BASE OUTFIT FROM BODY-FRONT / BODY-SIDE / BODY-BACK, EXACTLY. FOR A DERIVED SHEET: ONE APPROVED CURRENT OUTFIT ONLY]
...
The outer shoulder-top line must sit on exactly the same horizontal line as the CENTER panel.
...
The outer shoulder-top line must sit on exactly the same horizontal line as the LEFT panel.
The CENTER body must look like the LEFT body rotated exactly 180 degrees around its vertical axis.
...
Maximum-size frontal head-only portrait of the exact same woman, framed primarily from crown to chin.
...
Use the same hairstyle as the other panels. Cropping below the chin is allowed; shoulders, outfit and the rest of the body are not required to be visible.
...
3. 한 장에 한 복장만 있는가. BASE 시트라면 승인된 기본 복장을 정확히 유지했는가.
...
8. 두 전신 패널의 바깥쪽 어깨 상단선이 같은 수평선에 있는가.
9. 허리, 골반, 무릎, 발 높이가 두 전신 패널에서 대응하는가.
10. 정면·후면이 같은 체형으로 읽히는가.
11. PANEL 1에 얼굴·머리카락 정보가 완전히 제거되었는가.
12. PANEL 2에 얼굴이 보이지 않는가.
13. PANEL 3만 유일한 얼굴인가.
14. PANEL 3이 정수리~턱을 우선해 최대 크기로 담은 두상인가.
15. PANEL 3 얼굴이 FACE MASTER와 Morphology Card를 유지하는가.
16. PANEL 3의 헤어가 PANEL 2와 동일한가.
17. 신체가 BODY MASTER와 Body Signature를 유지하는가.
18. 복장만 바뀌고 모델 자체가 바뀌지 않았는가.
19. 손·발·관절·의상 구조가 자연스러운가.
20. 세 패널의 조명·배경·화이트밸런스가 동일한가.
...
`MODEL-001-MOTION-REFSHEET-BASE-v01.png`

지침 파일:

`seedance-2.5-3panel-reference-sheet-guide-v3.md`

## 12. 현재 승인 기준 사례

MODEL-001 차예린의 다음 두 시트는 v3에서 **레이아웃 전용 LEGACY FORMAT REFERENCE**로 사용한다. BASE 시트의 복장 규칙을 충족하는 정본 사례로 취급하지 않는다.
